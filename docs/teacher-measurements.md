...
| ID | The number | Sensor | How | Status |
| --- | --- | --- | --- | --- |
| **R7** | **Lesson start** — when the first learning task was set | Mic + Camera | The earlier of two signals: her first task-setting utterance (starter, do-now, retrieval, discussion) and her first WRITING or POINTING at the board — standing at the board does not count. Either alone starts the lesson; both within two minutes of each other raise it to observed | **Built** (rule set 2026-09-05): phrase patterns in both scripts for the voice signal (`lib/phrases.ts`), pointing/writing intervals for the board signal (`lib/lesson-arc.ts`); one signal = provisional, both agreeing = observed, neither = Not Observed. Her pointing/writing boxes are now stored with her teacher boxes so the signal survives a re-derive (lessons analysed before 2026-09-05 have none stored) |
