...
def _activity_features(
    ...
    Returns {"ts": torso_scale, "fc": facing_score,
             "arms": [[wrist_x, wrist_y, wrist_up_ratio], ...]} (0..2 arms that
    clear KPT_CONF_ARM), or None when no arm is usable. wrist_up_ratio =
    (shoulder_y - wrist_y) / torso_scale is >0 when the hand is raised. Wrist
    x/y stay in normalized coords so app.activity can test them against the
    (normalized) board polygon; the board-relative decision happens at derive.
    """
